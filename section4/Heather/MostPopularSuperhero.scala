...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    // Print out the most popular superhero
    println(s"$mostPopularName is the most popular superhero with ${mostPopular._1} co-appearances.") 
    
    // Print out the top 10 most popular superheroes
    // note: cannot perform map to add names to RDD since neither RDD has been collected
    val results = sorted.collect().take(10)
    for ( result <- results ) {
      println(namesRdd.lookup(result._2)(0))
    }
  }
